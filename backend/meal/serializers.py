...
class GuestMealOrderSerializer(serializers.ModelSerializer):
    ...
    meal_type_id = serializers.PrimaryKeyRelatedField(
        queryset=MealType.objects.all(), source="meal_type",
        write_only=True
    )
    ...
    class Meta:
        model = MealOrder
        fields = [
            "id", "date", "meal_type", "meal_type_id",
            "guest", "guest_id", "ordered", "auto_generated", "note", "weekday"
        ]
    ...
    def validate(self, attrs):
        ...
        if MealOrder.objects.filter(date=date, meal_type=meal_type, guest=guest).exists():
...
class StaffMealOrderSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            "id", "date", "meal_type", "meal_type_id",
            "staff", "staff_id", "ordered", "auto_generated", "note", "weekday"
        ]
    ...
    def validate(self, attrs):
        ...
        if MealOrder.objects.filter(date=date, meal_type=meal_type, staff=staff).exists():
            raise serializers.ValidationError({
